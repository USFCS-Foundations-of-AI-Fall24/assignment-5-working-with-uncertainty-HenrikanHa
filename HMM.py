   ...
   ## you do this.
    ...
    def forward(self, sequence, domain=""):
        ...
        for state in state_values:
            if state != '#':
                M[state_index[state], 0] = 0
                T_start = self.transitions['#'].get(state, 0)
                E_emit = self.emissions[state].get(sequence[0], 0)
        ...
        for t in range(2, num_observations + 1):
            for state in state_values:
                if state != '#':
                    sum_prob = 0
                    E_emit_prev = self.emissions[state].get(sequence[t - 1], 0)
                    for state2 in state_values:
                        if state2 != '#':
                            prev_prob = M[state_index[state2], t - 1]
                            T_trans = self.transitions[state2].get(state, 0)
    ...
    def viterbi(self, sequence):
        ...
        for t in range(2, num_observations + 1):
            for state in state_values:
                if state != '#':
                    E_emit = self.emissions[state].get(sequence[t - 1], 0)
                    max_val = 0
                    best_prev_state = 0
...
def main():
    ...
    if args.viterbi:
        ...
        most_likely_sequence = h.viterbi(sequence)
